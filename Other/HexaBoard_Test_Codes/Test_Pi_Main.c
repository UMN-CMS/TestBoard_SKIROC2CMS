...
int format_channels(){
	...
	for (int chip = 0; chip < 4; chip = chip +1 ){
        	for (int ch = 0; ch < 128; ch = ch +1 ){
        		for (int hit = 0 ; hit <13 ; hit = hit +1){
...
int prog384(int hexbd, unsigned char * pNew, unsigned char * pPrevious){
        ...
        unsigned char dout;
        for(int chip = 0; chip < 4; chip = chip+1){
                for(int bit = 0; bit < 384; bit = bit + 3){
...
///////////////////////////////////////////////////////////////////////////////////////////////	
	...
	for(int i = 0; i < 33000; i = i+1){
		for(int hexbd = 0; hexbd < MAXHEXBDS; hexbd = hexbd + 1){
...
/*****************************************************/
	...
	int hexbd = 0;// First Check that things work for 1 hexaboard, then loop over them.
	for(int i = 0; i < maxevents; i = i +1){
    		res = spi_send_command(hexbd, CMD_RESETPULSE);
    		usleep(10);
        ...
        /*****************************************************/
    		for(int chip = 0; chip < 4; chip = chip + 1){
    			...
    			for(int ch = 0; ch < 128; ch = ch +1){
    				for (int sample = 0; sample < 13; sample = sample +1){
