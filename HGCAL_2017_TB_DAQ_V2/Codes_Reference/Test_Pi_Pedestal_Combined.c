...
unsigned char tmp1_raw[RAWSIZE];
unsigned char tmp2_raw[RAWSIZE];
...
int main(int argc, char *argv[]){
	...
	int ch, sample, chip;
	int i, k;
	int raw_iterator, bit_position;
	time_t rawtime;
	bool saveraw;
	struct tm *info;
	char buffer[80];
	char fname [160];
	char dirname[100] = "/home/pi/HGCAL_2017_TB_DAQ/data/";//Change
...
///////////////////////////Setting up number of run events, file name, etc////////////////////////////////// 
	...
	runid = atoi(argv[1]);
	char runNum[12];
	sprintf (runNum, "PED_RUN_%04d", runid);
...
//	usleep(delay5) 
	...
	for(raw_iterator = 0; raw_iterator<RAWSIZE; raw_iterator++){
		...
		for(bit_position = 0; bit_position < 4; bit_position++){
			bool iii_HasFired_1 = (bool) (tmp1_raw[raw_iterator] & 1<<bit_position);
			bool iii_HasFired_2 = (bool) (tmp2_raw[raw_iterator] & 1<<bit_position);
			if(iii_HasFired_1){
                        	raw_32Bit[raw_iterator] = raw_32Bit[raw_iterator] | ((unsigned int) 1<<bit_position);
                	}
                	if(iii_HasFired_2){
                        	raw_32Bit[raw_iterator] = raw_32Bit[raw_iterator] | ((unsigned int) 1<<(bit_position+4) );
                	}
