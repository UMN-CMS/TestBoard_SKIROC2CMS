...
int progandverify48(int hexbd, unsigned char * pConfBytes, unsigned char * pPrevious){
        ...
        prog384(hexbd, pNewConfBits, pOldConfBits);
        int oldNewComparison = strcmp((char *)pNewConfBits, (char *)pOldConfBits);
        ConvertProgrStrBittoByte(pOldConfBits, pPrevious);
        free(pNewConfBits);
        free(pOldConfBits);
        if (oldNewComparison == 0) return(0);
        else return (1); // Return code 1: programming unsuccessful: at least one bit is different between prog string sent and received
}
...
int main(){
  ...
  unsigned char prog_string[PROGSTRLENGTH];
  int res, hexbd;
  int proginputhex1, proginputhex2, proginputhexcombined;

  FILE *progFile;
  progFile = fopen("progStr.txt", "r");
  if (progFile == NULL) {
    printf("Unable to open file!\n");
    exit(1);
  }
  // fscanf(progFile, "%s", prog_string);
  // unsigned progCounter;
  for (unsigned progCounter = 0; progCounter < PROGSTRLENGTH; ++progCounter) {
    fscanf(progFile, "%1X%1X", &proginputhex2, &proginputhex1);
    proginputhexcombined = (0x10)*proginputhex2 + proginputhex1;
    printf("Configuration int read in: %d\n", proginputhexcombined);
    unsigned char proginputhexchar = (unsigned char)proginputhexcombined;
    sprintf(&(prog_string[progCounter]), "%c", proginputhexchar);
  }
  fclose(progFile);
  printf("Prog str read in:");
  int i;
  for(i = 0; i < PROGSTRLENGTH; ++i){
    printf("%x ",prog_string[i]);
  }
  printf("\n");
  ...
  for(hexbd = 0; hexbd < MAXHEXBDS; hexbd = hexbd + 1){
    ...
    int progandverifystatus = progandverify48(hexbd, prog_string, return_string);
    printf("\nFinished Configuration of Hexaboard %i \n", hexbd);
    if (progandverifystatus != 0) {
      exit(progandverifystatus);
    }
    (void) res;
  }
