...
int read_raw_faster(int hexbd) {
   ...
   for (i=0; i<30; i=i+1) { // Get 30000 values
      ...
      for (j=0; j<1000; j++) raw[k++] = (unsigned char) (result[j] & 255);
   }
   for (i=0; i<3; i=i+1) { // Get 600 values.
      status = spi_read200_local_fifo(hexbd, &(result[0]));
      for (j=0; j<200; j++) raw[k++] = (unsigned char) (result[j] & 255);
   }

   // Clean up the last stragglers.
   for (i=0; i<(RAWSIZE-30000-6000); i=i+1) {
	   t = spi_read_local_fifo(hexbd);
	   raw[i] = (unsigned char) (t & 255);
   }
   return(0);
...
int main(int argc, char *argv[]){
	...
	char buffer[80];
	char fname [160];
	char dirname[] = "/home/pi/HGCAL_2017_TB_DAQ_Test/data/";//Change
...
//                 res = spi_send_command(hexbd, CMD_SETSTARTACQ);  // <<<+++   THIS IS THE TRIGGER
		 res = spi_clear_busy(hexbd); // <<<<<<< Hardware trigger
