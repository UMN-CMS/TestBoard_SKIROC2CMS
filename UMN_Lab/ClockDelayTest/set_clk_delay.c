//--------------------------------------------------------------------
// COMPILE: 
// gcc -g set_clk_delay.c -o set_clk_delay -l bcm2835 -Wall -std=c11
//--------------------------------------------------------------------
// USAGE: 
// sudo ./set_clk_delay [Chip Number] [Delay Value in picoseconds]
//--------------------------------------------------------------------
//	Set a single clock delay
//--------------------------------------------------------------------
...
int main (int argc, char *argv[]) {
	...
	int usr_delay = atoi(argv[2]);// user should enter desired delay in picoseconds
	...
	char GPIO, IODIR;
	int clk_del_select = atoi(argv[1]);
	if(clk_del_select < 4) {
	...
	for(bitnum = 0; bitnum < REG_LEN-2; bitnum++) {
		to_send[bitnum+2] = (delay >> bitnum) & 1;
	}
